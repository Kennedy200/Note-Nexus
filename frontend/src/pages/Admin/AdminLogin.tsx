      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-[2.5rem] p-10 border border-white/20 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            >
              {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Signing In...</> : <><Shield className="w-5 h-5" /> Sign In to Admin</>}
            </button>
          </form>

          <p className="text-center text-blue-300 font-bold mt-8 text-sm">
            No admin account?{' '}
            <Link to="/admin/signup" className="text-white underline hover:text-blue-200">
              Create Admin Account
            </Link>
          </p>

          <p className="text-center mt-4">
            <Link to="/login" className="text-blue-400 font-bold text-sm hover:text-blue-300">
